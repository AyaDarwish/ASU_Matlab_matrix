...
CMatrix::CMatrix( string name, int rn, int cn, int initial, double initialvalue)
{
	setName(name);
	rowsNumbers = rn;
	coloumnsNumbers = cn;
...
CMatrix CMatrix::operator + (CMatrix& m)
{
	...
	{
		CMatrix r(matrixName, this->rowsNumbers, this->coloumnsNumbers, 0, 0);
...
CMatrix CMatrix::operator - (CMatrix& m)
{
	if (rowsNumbers != m.rowsNumbers || coloumnsNumbers != m.coloumnsNumbers)
	{
		...
		CMatrix r (this->matrixName, rowsNumbers, coloumnsNumbers, 0, 0);
		for (int iR = 0;iR<rowsNumbers;iR++)
			for (int iC = 0;iC<coloumnsNumbers;iC++)
			{
				r.matrix[iR][iC] = this->matrix[iR][iC] - m.matrix[iR][iC];
...
CMatrix CMatrix :: operator * (CMatrix &x)
{
	...
	{
		CMatrix rere (this->matrixName, rowsNumbers, x.coloumnsNumbers, 0, 0);
...
CMatrix CMatrix:: operator / (CMatrix& m)
{
	...
	CMatrix r((to_string(y).c_str()), this->rowsNumbers, m.coloumnsNumbers, 0, 0);y++;
...
CMatrix elementDivision(CMatrix &m, CMatrix &c)
{
	...
	{
		CMatrix r ((to_string(y).c_str()) , m.rowsNumbers, m.coloumnsNumbers, 0, 0);y++;
...
CMatrix elementoperations(CMatrix m, CMatrix c,char op,int precedence) // op==>operator
{
	 
	CMatrix r((to_string(y).c_str()), m.rowsNumbers, m.coloumnsNumbers, 1, 1);y++;
	CMatrix k((to_string(y).c_str()), c.rowsNumbers, c.coloumnsNumbers, 1, 1);y++;
	...
	{ 
		for (int ic = 0;ic<m.coloumnsNumbers;ic++)
		{
			switch (op)
			{
			case '+':{ r.matrix[ir][ic] = m.matrix[ir][ic] + c.matrix[0][0];break;}
			case '-':{ r.matrix[ir][ic] = m.matrix[ir][ic] - c.matrix[0][0];break;}
			case '*':{ r.matrix[ir][ic] = m.matrix[ir][ic] * c.matrix[0][0];break;}
			case '/':
			{
				...
				if (precedence==1)
					{
						...
						for (int i = 0;i<c.rowsNumbers;i++)
						{
							
		                 for (int j = 0;j<c.coloumnsNumbers;j++)
						 {

						    k.matrix[i][j] = m.matrix[0][0] / c.matrix[i][j]; // Const/Matrix
			...
			case '^': {r.matrix[ir][ic] = pow(m.matrix[ir][ic], c.matrix[0][0]);break;}
			}
